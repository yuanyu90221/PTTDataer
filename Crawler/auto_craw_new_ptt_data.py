...
def craw_ptt_data_fun(article_url,
    ...
    # article_url = 'https://www.ptt.cc/bbs/NBA/M.1526380904.A.17A.html'
        ...
        soup2 = BeautifulSoup(res2.text, "lxml")# beautiful 漂亮的呈現原始碼
    # author                
    author = temp[j].find("",{'class':'author'}).get_text() # 抓出作者
    # date
    ...
    # title
    title = temp[j].find('a').get_text()# 抓文章 title
    #print(j,title)        
...
def main_craw_ptt(i,ptt_class_name,sql_name,bo):
    ...
    for j in range( len( temp ) ): # i=3 len( temp )
        #print(i)
        temp2 = temp[j].find('a')
        if( str( temp2 ) == 'None' ):# 如果該網址是空   則 return error, 因為有可能對方已刪文
            ...
            article_url = temp[j].find('a')['href']# 抓文章網址
            article_url = index_name+article_url# 與 index 合併
            title = temp[j].find('a').get_text()# 抓 title
            #print(i,title)
            ...
            if( response.status_code == 404 ):
                ...
                craw_ptt_data_fun(article_url,temp,j,index_url,
...
def auto_craw_new_ptt_data(amount,ptt_class_name,sql_name):    
    ...
    #data = pd.DataFrame()
    for i in range(last_index-amount,last_index+1,1):# i = 16546
        k = 0
        print(i,'================================================')
        # connect wrong
        while(k==0):
...
#amount_set = [ 20*i for i in amount_set ]

if __name__ == '__main__':
    for i in range(len(ptt_class_name_set)):# i = 107
        #print(i)
        ptt_class_name = ptt_class_name_set[i]
        sql_name = sql_name_set[i]
        amount = amount_set[i]
        print(ptt_class_name)
